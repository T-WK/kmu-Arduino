...
float dist_min, dist_max, dist_raw, dist_pre; // unit: mm
...
// get a distance reading from the USS
  dist_pre = USS_measure(PIN_TRIG,PIN_ECHO);
  if (dist_pre != 0){
    dist_raw = dist_pre;
  }
...
// turn on the LED if the distance is between dist_min and dist_max
  if(dist_min <= dist_raw && dist_raw < 200) {
    analogWrite(PIN_LED, 254 - (int)(254/100.0*(dist_raw-100)));
  }
  else if(200 <= dist_raw && dist_raw < dist_max) {
    analogWrite(PIN_LED, (int)(254/100.0*(dist_raw-100)));
  }
